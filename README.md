...
- **Security headers** on every response (`X-Content-Type-Options`, `X-Frame-Options`, `Referrer-Policy`, a strict CSP, and `Permissions-Policy`). The CSP allows `wasm-unsafe-eval` and `unsafe-inline` for scripts, which the Dioxus WASM bootstrap loader requires.
...
#                the dioxus CLI (`cargo binstall dioxus-cli --version 0.6.3`)
...
./scripts/build-frontend.sh   # build the Dioxus WASM frontend into frontend/dist/
cargo run --release           # backend embeds frontend/dist/ at compile time
```
