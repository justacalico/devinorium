...
/// The directory that holds all managed worktrees for a repo.
///
/// Worktrees are grouped in a hidden `.<repo>-worktrees` directory under the
/// configured `root` so they do not live inside the main working tree and
/// risk being nested.
pub fn managed_worktrees_root(toplevel: &Path, root: &Path) -> PathBuf {
    let dir_name = toplevel
        .file_name()
        .map(|s| s.to_string_lossy().to_string())
        .unwrap_or_else(|| "repo".to_string());
    root.join(format!(".{dir_name}-worktrees"))
}

/// Return the managed worktree directory for a repo and branch.
pub fn managed_worktree_path(toplevel: &Path, branch: &str, root: &Path) -> PathBuf {
    managed_worktrees_root(toplevel, root).join(sanitize_branch_for_path(branch))
}
...
pub fn is_temporary_worktree_branch(branch: &str) -> bool {
    static RE: std::sync::LazyLock<regex::Regex> = std::sync::LazyLock::new(|| {
        regex::Regex::new(TEMP_WORKTREE_BRANCH_PATTERN).expect("valid regex")
    });
...
impl GitService {
    ...
    pub async fn prune_worktrees(&self, path: &Path) -> Result<(), GitError> {
        self.repo_status(path, false).await?;
...
mod tests {
    ...
    fn managed_worktree_path_is_outside_repo() {
        let path = std::path::PathBuf::from("/srv/repos/project");
        let root = std::path::PathBuf::from("/home/user");
        let wt = managed_worktree_path(&path, "devinorium/abc12345", &root);
        assert_eq!(
            wt,
